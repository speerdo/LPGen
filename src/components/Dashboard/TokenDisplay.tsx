...
import { PLANS, TOKEN_COSTS } from '../../services/tokens';
import { Info, AlertCircle, Coins, RefreshCw } from 'lucide-react';
...
const TokenDisplay: React.FC<TokenDisplayProps> = ({ compact = false }) => {
  const { userSubscription, refreshSubscription } = useAuth();
  ...
  const fullPagesRemaining = Math.floor(
    token_balance / TOKEN_COSTS.INITIAL_LANDING_PAGE
  );
  const editOperationsRemaining = Math.floor(
    token_balance / TOKEN_COSTS.CONTENT_UPDATE
  );
  ...
  const handleRefresh = async () => {
    ...
    try {
      // Force refresh without parameters
          ...
          <div className='absolute right-0 mt-2 w-64 bg-white rounded-md shadow-lg z-50 border border-gray-200 p-3'>
            ...
            <div className='text-xs text-gray-500 mb-2'>
              {fullPagesRemaining > 0
                ? `~${fullPagesRemaining} landing page${
                    fullPagesRemaining !== 1 ? 's' : ''
                  } or ${editOperationsRemaining} edits remaining`
          ...
          <div>
            ...
            <div className='text-xs text-gray-500 mt-1'>
              {fullPagesRemaining > 0
                ? `~${fullPagesRemaining} landing page${
                    fullPagesRemaining !== 1 ? 's' : ''
                  } or ${editOperationsRemaining} edits`
                : 'Purchase more tokens to continue'}
            </div>
          </div>
          ...
          <ul className='text-xs space-y-1 text-gray-700'>
            <li className='flex justify-between'>
              ...
              <span className='font-medium'>
                {TOKEN_COSTS.INITIAL_LANDING_PAGE} tokens
              </span>
            </li>
            <li className='flex justify-between'>
              <span>Page refinement:</span>
              <span className='font-medium'>
                {TOKEN_COSTS.LANDING_PAGE_REFINEMENT} tokens
              </span>
            </li>
            <li className='flex justify-between'>
              <span>Content update:</span>
              <span className='font-medium'>
                {TOKEN_COSTS.CONTENT_UPDATE} tokens
              </span>
            </li>
            <li className='flex justify-between'>
              <span>Section generation:</span>
              <span className='font-medium'>
                {TOKEN_COSTS.SECTION_GENERATION} tokens
              </span>
            </li>
          </ul>
