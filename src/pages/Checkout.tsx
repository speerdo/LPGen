  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          'Supabase function checkout failed, trying direct checkout:',
          supabaseError
        );
      ...
      const sessionId = await createDirectCheckoutSession(
        item.priceId,
        user.id,
        item.isSubscription
      );
          ...
          <div className='bg-white shadow overflow-hidden rounded-lg'>
            ...
            <div className='px-6 py-5'>
                    ...
                    <div className='ml-3'>
                      ...
                      <ul className='mt-2 text-sm text-blue-700 space-y-1'>
                        <li>
                          •{' '}
                          {Math.floor(
                            item.tokens / TOKEN_COSTS.INITIAL_LANDING_PAGE
                          )}{' '}
                          new landing pages
                        </li>
                        <li>
                          •{' '}
                          {Math.floor(
                            item.tokens / TOKEN_COSTS.LANDING_PAGE_REFINEMENT
                          )}{' '}
                          page refinements
                        </li>
                        <li>
                          •{' '}
                          {Math.floor(item.tokens / TOKEN_COSTS.CONTENT_UPDATE)}{' '}
                          content updates
                        </li>
                      </ul>
                    </div>
              ...
              <div className='mt-4 text-center text-xs text-gray-500'>
                ...
                <p className='mt-1'>
                  Your card will be{' '}
                  {item.isSubscription ? 'billed monthly' : 'charged once'}.
                </p>
              </div>
            </div>
          </div>
