...
import { getProjects, deleteProject, getProjectVersions } from '../lib/supabase';
...
function Dashboard() {
  ...
  function handleDownload(html: string, fileName = 'landing_page.html') {
  ...
  async function handleProjectDownload(project: Project) {
    ...
    if (!htmlContent) {
      try {
        ...
        console.error('Error fetching project version for download', error);
      }
    }
    handleDownload(htmlContent, `${project.name}_landing_page.html`);
  }
